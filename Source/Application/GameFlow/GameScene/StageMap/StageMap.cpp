...
namespace app {
	void StageMap::startForRood() {
		...
		if (isState(0)) {
			setMapObjNum(1);
			cam_pos.set(AREA::FMAX_X / 2 - FMIN_X - 100, 1400.0f, -80.0f);
			...
			obj_pos.set((FMAX_X - FMIN_X) / 2, FMAX_Y / 2, 0);
		...
		if (isState(3)) {
			cam_pos.set(AREA::FMAX_X / 2 - FMIN_X - 100, 1800.0f, -80.0f);
			...
			obj_pos.set(AREA::FMAX_X / 2, AREA::FMAX_Y / 2, 0);
		...
		if (isState(5)) {
			if (m_Stage_count <= 3162) {
				cam_pos.set(AREA::FMAX_X / 2 - FMIN_X - 100, 3400.0f, -80.0f);
				...
				obj_pos.set((FMAX_X - FMIN_X) / 2 + FMIN_X, (FMAX_Y - FMIN_Y) / 2, 0);
